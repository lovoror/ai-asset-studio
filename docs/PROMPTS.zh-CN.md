...
- 每张参考图会先复制进**本任务**目录（`<job>/references/ref0.<ext>`、`ref1.<ext>`），因为只有任务目录下的文件才会被发往另一台机器上的 worker；
  而且必须放在**阶段目录之外**——阶段目录被 runner 视为「本阶段的产物」：其中的路径会被写成 `@out/...` 占位符并且**不会**被上传，
  而任务目录内的路径会变成 `@in/...` 并被上传（见 `services/studio/studio/runner_client.py` 的 `StagePlan`）。
