...
| 画布（`VIEWS_SIZE`） | 2048×512，即 **4:1** | 见下——这才是模型肯排成一排的原因 |
...
* **模型还会把它吃的源图也画进那一排。** 节点包把源图按"居中偏移"重采样到目标画布，所以多出来的那一格落在正中间——
  真实任务里离中线 **2px**，而最近的真实视角离中线 360px。`studio/sheet.py` 按位置剔除它，并且**不做逐格归一化**
  （四个视角共用同一个取景窗，尺寸取最宽那格；否则窄的正视图会被放大到跟宽的侧视图一样大），而且每格只从自己那块面板
  取像素，邻居视角不会漏一条边进来。
