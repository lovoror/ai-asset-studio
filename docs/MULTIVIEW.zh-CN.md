...
* **左右两侧用"物体在画面里朝向哪边"来描述，画完还要再验一次。** 提示词写的是**画面方向**（"侧视，物体朝向这一格的左边"），
  这正是机架的意思：§2 的方位角把 left 相机放在 +x，那里画面右侧是 +y 而物体朝向 −y，所以"物体的左侧"正是**车头朝画面左边**的那张侧视。
  这个说法是为了修一张"左右画反"的图条才改的——但那张图条同时也来自会把源图画进画布的工作流，所以归因是错的；后来的实测也不支持它：
  在一个模型总是画错的参考图上试了四种说法，给出正确左右对的两种各只成功了一次，换三个种子后又都失败了。**目前没有任何说法被证明能解决这件事**，
  所以措辞保持原样。除此之外，画完之后还会读这一对（不翻转 vs 翻转），发现是同一侧画了两次就把右视图镜像过来
  （实测：重复对不翻转相关 0.99、翻转 0.76；正确镜像对翻转后 0.95）。重复的对等于就物体的一半告诉机架两件互相矛盾的事——
  "车不是车"有一半来自这里；另一半来自左右互换，那等于把物体按镜像重建。
* **"模型到底会不会画另一侧"有相当一部分取决于参考图本身。** 某张参考图实测：试的四个种子里有两次给出正确的一对（所以重画有用）；
  另一张参考图则**每一个种子、每一种措辞**（一共七次）都画成同一侧——那就不是"重画"能解决的，而是没有可画的东西。
  流水线会读切图器修了什么（`studio/sheet.py` 通过 `notes` 返回 `side_pair`、`repeated`、`redo`），只要修过就**重画**，
  最多 `TURNAROUND_ATTEMPTS`（2）次，每次把种子往前推 `TURNAROUND_RESEED_STEP`；被放弃的图条会以 `turnaround_attempt<N>.png`
  留在最终图条旁边，因为"重画到底画出不一样的东西了吗"是第一个要问的问题。只有最终采用那张图条的告警会被上报，
  重画次数记在任务的 `turnaround` 结果里。如果重画后仍然是同一侧，就只能镜像——对左右对称的物体精确，对不对称的物体是近似。
  其实也可以**另外画一张真正的另一侧**（`var/try_turnaround.py` 用 1024×1024 + 单视角提示词，约一分钟就能画出来），
  但目前没有接进流水线：一张图条里的四个视角共用同一个尺度，另外画的那张必须按图条里物体的高度缩放才能对齐。
...
* **四个视角里"右侧"仍然最弱，而且在某些参考图上根本画不出来。** 模型把侧视画成四分之三视角的概率在右侧更高；
  而在某张参考图上，无论换种子还是换措辞，它都把两侧画成同一侧——只剩镜像这一条退路。把那一侧**单独画一张**是可行的（见 §7），
  也是最自然的下一步；目前没接进来，是因为一张图条里的四个视角共用同一个尺度，单独画的那张必须先缩放到一致才行。
* **目前只检查"某一格不是另一格的复制"，不检查"画的是不是那个视角"。** 重复检查能抓出"同一视角画了两次"（背面槽位拿到侧视就是这么来的），
  但如果那一格画成了**另一个错的视角**（要后视却给了个后四分之三），它查不出来；两侧以同样方式画错，它也查不出来。
