...
* **左右两侧用"物体在画面里朝向哪边"来描述，画完还要再验一次。** 写成"第二格是左侧视"时，实测出现过左右**画反**：
  正、后正确而两侧互换——"只有前后是对的"这个反馈就是这个样子。所以提示词改成用**画面方向**描述
  （"侧视，物体朝向这一格的左边"）。这不是咬文嚼字，而是机架本身的意思：§2 的方位角把 left 相机放在 +x，
  那里画面右侧是 +y 而物体朝向 −y，所以"物体的左侧"正是**车头朝画面左边**的那张侧视。除此之外，画完之后
  还会读这一对（不翻转 vs 翻转），发现是同一侧画了两次就把右视图镜像过来（实测：重复对不翻转相关 0.99、翻转 0.76；
  正确镜像对翻转后 0.95）。重复的对等于就物体的一半告诉机架两件互相矛盾的事——"车不是车"有一半来自这里；
  另一半来自左右互换，那等于把物体按镜像重建。

**"哪些格子是视角"是同一个问题的另一面，这里也踩过一次。** 最直觉的规则——整张图取一个背景色、和它不同的列连成一段
就是一格——在真实图条上不成立，因为各格背景并不同色：四个生成视角落在灰色渐变幕布上，而模型自绘的源图那一格是白底。
于是"边缘出现最多的颜色"取到了白色，另外四格所有灰色列都被算成物体，整排被并成**两大格（各 1022px、每格里有两辆车）**，
其中一格还被当成 "front" 送进了重建。现在改成按**物体自身的边缘**切格：幕布和影子都是平滑的，物体边界一定是突变，
而且不管背景是什么颜色都成立。两张真实图条实测：阈值从 10 到 30 都切出同样的五格；再低就会因为影子自身的软边（6–9）
把相邻两格粘在一起。
...
* **四个视角里"右侧"仍然最弱。** 模型把侧视画成四分之三视角的概率在右侧更高，所以那一格最可能需要调 `views_prompt`。
  目前也没有任何检查能确认那一格画的确实是提示词要的那一侧；旁边的镜像检查只能判断"这一对不是同一侧画了两次"。
