...
13. **无限画布 `/canvas`**（P2，2D 生成的第二种做法）：一块自由平移缩放的板子，每张**卡片**就是一次生成请求。
    卡片上直接显示流水线**实际会发出去的提示词**——`POST /v1/prompt/preview` 的组装结果，可展开看它由哪些部分组成——
    并用「合成 / 我的文本」开关明确当前发的是哪一份（切到「我的文本」会用组装好的文本预填输入框，改起来从默认值开始）。
    每张卡片可带 **0～6 张参考图且顺序即语义**（双图编辑里第一张是主体、第二张是要模仿的对象），参考图可以上传本地图片，
    也可以直接挑之前某个方案/卡片里的图；结果留在卡片上，可点开大图（复用 Lightbox）、可「作为参考图」开一张新卡片继续迭代、
    也可直接「生成 3D」。整块画布（卡片、位置、视图、上传的图）存在浏览器本地存储里，刷新即恢复；平移缩放是手写的，没有引入新依赖。
    ...
    还没做（P3）：多视角卡片、卡片之间连线、服务端保存画布、预设库。详见 [`docs/PORTAL.md`](docs/PORTAL.md) 的 Canvas 一节。
