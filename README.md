...
12. **提示词看得见，参考图可以多张**（无限画布的第一阶段，后端部分）：`POST /v1/prompt/preview` 返回真正会发出去的提示词
    ...
    画布界面见下面第 13 条：接口这一侧就是为它准备的。
13. **无限画布 `/canvas`**（节点图，2D 生成的第二种做法）：一块自由平移缩放的板子，上面是**节点**而不是表单——
    提示词、风格、参考图各自是**独立节点**，用**连线**喂给「生成 2D」节点，结果再连给「生成 3D」节点。
    拆开的原因：一张卡片把提示词/风格/参考图绑在一起，就只能描述一次请求；拆开之后，一个风格节点可以喂给任意多个生成节点，
    一张参考图可以被任意多次生成复用，而把上一张结果接进下一次生成只是**拉一根线**，不是复制整份请求。
    端口带类型（`prompt`/`style`/`image`），只能同类相连；「生成 2D → 生成 2D」是合法的（在同一张图上继续迭代），成环会被拒绝；
    只收一根线的输入再连一根会**替换**原来那根，参考图这类多输入按连接顺序排列并标出序号——顺序即语义（双图编辑里第一张是主体、
    第二张是要模仿的对象）。「生成」节点自己没有提示词和风格，全部取自连线，没连时会直接说明；只有提示词是必需的，
    风格没连则回落默认值（端口上写明），参考图可选。节点上仍然显示流水线**实际会发出去的提示词**
    （`POST /v1/prompt/preview` 的组装结果，组成部分可展开），「合成 / 我的文本」开关决定发哪一份，
    切到「我的文本」会用组装好的文本预填输入框（改起来从默认值开始）。结果留在节点上，可点开大图（复用 Lightbox）、
    可直接「生成 3D」，也可以走「生成 3D」节点。整块画布（节点、连线、视图、上传的图）存在浏览器本地，刷新即恢复；
    旧版「一卡片一请求」的画布在加载时会**迁移**成节点图，不会丢。平移缩放是手写的，没有引入新依赖。
    提示词需要参考图时，若当前配置的图像工作流是文生图（没有 LoadImage，例如默认的 `KREA-2-TURBO.json`），
    流水线会**自动改用**内置的 `krea2_edit_refs.json` 并把这次替换写进任务警告——否则画布上没有任何地方能改这个全局设置。
    还没做：服务端保存画布、预设库、多视角节点。详见 [`docs/PORTAL.md`](docs/PORTAL.md) 的 Canvas 一节。
